      ...
      for(bt3 in 1:nrow(true_bta)){
        for(bt4 in 1:nrow(true_bta)){
          ...
          dat <- data.table::CJ(
            ...
            true_beta = NA_real_,
            est_beta = NA_real_,
            true_exp_beta = NA_real_,
            est_exp_beta = NA_real_,
            est_exp_beta_sd = NA_real_,
            est_sample =  NA_real_,
            est_sd =  NA_real_
          )
          ...
          dat$true_exp_beta = rep(c(qlogis(true_pr[pr]),true_bta[bt2,2],true_bta[bt3,3],true_bta[bt4,4]),length(n_sample))
          for(j in 1:length(n_sample)){
            ...
            dat[dat$interim==j,"est_exp_beta_sd"] <- apply(exp(res$post.beta.mn[,j,]),1,sd)
            #
